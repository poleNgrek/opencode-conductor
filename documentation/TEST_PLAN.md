...
**Pass**: all mermaid behavior matches the kit-wide policy in `documentation/PATH_CONTRACT.md` § Mermaid policy.
...
**Expected**: each has `subtask: true` per the kit-wide convention table in `documentation/PATH_CONTRACT.md` § Frontmatter conventions; long advisory output stays out of the primary context window when run as a subtask.
...
**Pass**: the kit-contract rule from `documentation/PATH_CONTRACT.md` § Security rules is upheld kit-wide.
...
- [ ] Frontmatter conventions: `subtask: true` set on long-output commands per the table in `documentation/PATH_CONTRACT.md`
