...
- The kit is installed via `bin/install-opencode-conductor.sh`.
...
| Manual refresh | `/manual-refresh <key>` | same shape; no tools required | engine unavailable error |
| Bootstrap | `/project-bootstrap <key>` | seeds `MERGE_REQUEST.md`, `LOG.md`, optional `PHASES.md` | refuses if files exist and not asked to merge |
| Scaffold knowledge | `/scaffold-knowledge <key>` | shared/leaf `AGENTS.md` created at convention paths | `invalid_package_name`, `symlink_refused`, `path_outside_root`, `source_missing` |
...
| Review | `/project-review <key>` | `REVIEW.md` produced; deterministic verifications from area `## Verification scripts` | missing-block `F-xx` note |
