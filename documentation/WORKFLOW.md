...
**Canonical ordered procedures** for this kit. For **which slash command when**, see [`documentation/COMMAND_WORKFLOW.md`](documentation/COMMAND_WORKFLOW.md). For **install, descriptor fields, cost, rules list**, see [`README.md`](README.md).

Replace **`<projectKey>`** with your descriptor key. The descriptor file is always loaded from **`~/.config/opencode/projects/<projectKey>/descriptor.json`** by kit tools; branch folders and `AGENTS.md` paths are **whatever `branchHandoff` and `areas` say** (often the same global tree, or a **project-local** tree — see [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md)).
...
> **Convention path for leaves.** With `descriptorSchemaVersion: 2`, leaf `AGENTS.md` files live at `<opencodeProjectRootPath>/<rel>/AGENTS.md`, where `<rel>` mirrors the leaf's path under `projectRootPath` (the **source-tree-mirror** convention). See [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md) for the stem derivation contract.
...
> The kit never writes through symlinks, never overwrites existing `AGENTS.md`, and rejects any path that escapes `opencodeProjectRootPath`. See [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md) for the complete safety guardrails.

---

## 12. Knowledge across branches

`AGENTS.md` files describe **durable** knowledge. Two storage modes affect how that knowledge moves between branches and how the kit detects discrepancies. Full details live in [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md) § Knowledge across branches; this section is the operational summary.
...
A short worked example lives in [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md) § AGENTS.md conflict playbook.
...
See [`documentation/PATH_CONTRACT.md`](documentation/PATH_CONTRACT.md) § Opt-out flags for the kit-wide table.
